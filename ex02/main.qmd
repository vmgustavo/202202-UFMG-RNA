...
```{python}
aux0 = np.linalg.pinv(
    np.matmul(hidden_output.T, hidden_output)
    + (reg * np.identity(neurons_hidden + 1))
)
aux1 = np.matmul(np.linalg.pinv(aux0), hidden_output.T)
output_weights_reg = np.matmul(aux1, target)
```

**Cálculo dos valores de saída da rede**

```{python}
output_input = np.matmul(
    hidden_output,
    output_weights
)
output_input.shape
...
class ELMClassifier:
    ...
    def fit(self, X, y):
        ...
        self.w_output_ = np.matmul(np.linalg.pinv(hidden_output), y)
